...
#define __SFC_SIMULAZIONETASTOEMERGENZA 11
...
void PROGRAM0_body__(PROGRAM0 *data__) {
  ...
  // FUSTELLA action associations
  {
    ...
    if (active && __time_cmp(data__->FUSTELLA.T.value, __time_to_timespec(1, 0, 2, 0, 0, 0)) >= 0) 
                      {__SET_VAR(data__->,__action_list[__SFC_SIMULAZIONETASTOEMERGENZA].state,,1);}
    else if (desactivated)
                      {__SET_VAR(data__->,__action_list[__SFC_SIMULAZIONETASTOEMERGENZA].state,,0);};
